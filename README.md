...
1. **DB 스키마 생성**: Supabase 대시보드 > SQL Editor에서 [`database/schema.sql`](database/schema.sql) 내용을 실행하세요. `subscriptions` 테이블과 RLS 정책, Realtime publication이 생성됩니다.
...
src/
  app/
    ...
    page.tsx             # 메인 화면 (캘린더뷰)
    register/            # 구독 서비스 등록 화면
    register/complete/   # 등록 완료 화면
  components/            # Sidebar, MonthCalendar, RegisterModal 등
  data/koreanSubscriptions.ts  # 국내 인기 구독서비스 30종 + 유사도 추천 로직
  hooks/                 # useSession, useSubscriptions (Supabase 연동/실시간 반영)
...
- 월~일 기준 월별 캘린더에 구독 서비스 결제일 표시 (주/월/년 단위 반복 결제 지원)
- 이번 달 구독료 합계, 3일 이내 결제 예정 알림
- 구독 서비스 이름 입력 시 국내 인기 서비스 30종과의 유사도 비교로 자동 추천
- 결제 금액 3자리 콤마 자동 포맷팅
